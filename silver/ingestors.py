...
class Ingestor:
    def __init__(self, spark, catalog, schema, output_table_name, input_format):
        self.spark = spark
    ...
    def _set_schema(self):
        ...
        schema_yml = self._open_yml(self.output_table_name)
        try:
    ...
    def run(self):
        print(f'Loading and transforming {self.output_table_name}')
        df = self.load()
...
class IngestorCDC(Ingestor):
    def __init__(self, spark, catalog, schema,  output_table_name, input_format):
        super().__init__(spark, catalog, schema, output_table_name , input_format)
        self._set_delta()
        self._set_fields()
